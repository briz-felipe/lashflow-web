...
export default function AgendarSlugPage() {
  ...
  const [clientData, setClientData] = useState({ name: "", phone: "", email: "", instagram: "" });

  // Load salon info
  useEffect(() => {
    api
      .get<SalonInfo>(`/public/salon/${slug}`)
      .then(setSalon)
      .catch(() => setSalonNotFound(true));
  }, [slug]);

  // Load procedures once salon is confirmed
  useEffect(() => {
    if (!salon) return;
    api
      .get<Procedure[]>(`/public/procedures?slug=${slug}`)
      .then(setProcedures)
  ...
  if (salonNotFound) {
    return (
      <div className="max-w-lg mx-auto text-center py-16">
        ...
        <h1 className="text-xl font-bold text-foreground mb-2">Salão não encontrado</h1>
        <p className="text-muted-foreground text-sm">
          O link que você acessou não corresponde a nenhum salão cadastrado.
  ...
  return (
    <div className="max-w-2xl mx-auto">
      ...
      <div className="text-center mb-8">
        <h1 className="text-2xl sm:text-3xl font-bold text-brand-800 mb-1">
          {salon.salonName ?? "Agendar Horário"}
        </h1>
        {salon.salonAddress && (
        ...
        {!salon.salonName && (
          <p className="text-muted-foreground text-sm">Escolha seu procedimento e horário preferido</p>
        )}
      </div>
        ...
        <div>
          ...
          <div className="grid grid-cols-7 gap-1.5 mb-5">
            {days.map((day) => {
              ...
              return (
                ...
                >
                  <span className={`font-medium uppercase tracking-wide text-[10px] ${isSelected ? "text-white/80" : "text-muted-foreground"}`}>
                    {format(day.date, "EEE", { locale: ptBR })}
                  </span>
                  <span className="font-bold text-sm leading-tight">{format(day.date, "d")}</span>
                  {day.loading ? (
          ...
          {!selectedDay && (
            <p className="text-sm text-muted-foreground text-center py-4">
              Selecione uma data com vagas disponíveis
            </p>
          )}
        ...
        <div>
          ...
          <div className="bg-white rounded-2xl border border-brand-100 shadow-card p-6 space-y-4">
            <div>
              ...
              <Input id="name" value={clientData.name} onChange={(e) => setClientData((d) => ({ ...d, name: e.target.value }))} placeholder="Seu nome" className="mt-1.5" />
            </div>
            <div>
              <Label htmlFor="phone">WhatsApp *</Label>
              <Input id="phone" value={clientData.phone} onChange={(e) => setClientData((d) => ({ ...d, phone: e.target.value }))} placeholder="(11) 99999-9999" className="mt-1.5" />
            </div>
            <div>
              <Label htmlFor="email">E-mail</Label>
              <Input id="email" type="email" value={clientData.email} onChange={(e) => setClientData((d) => ({ ...d, email: e.target.value }))} placeholder="seu@email.com" className="mt-1.5" />
            </div>
            <div>
              <Label htmlFor="instagram">Instagram</Label>
              <Input id="instagram" value={clientData.instagram} onChange={(e) => setClientData((d) => ({ ...d, instagram: e.target.value }))} placeholder="@seuusuario" className="mt-1.5" />
            </div>
          </div>
          ...
          <div className="flex gap-3 mt-4">
            ...
            <Button className="flex-1" disabled={!clientData.name || !clientData.phone || submitting} onClick={handleSubmit}>
